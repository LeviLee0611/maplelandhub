...
> 마지막 업데이트: 2026-07-16
> `posts`/`applications`(파티 구인) 테이블은 **레거시** — 파티 매칭 기능 자체는 라우트/컴포넌트/타입까지 전부 삭제됨(`27913d9` 이전 커밋). 테이블/RLS만 정리 안 되고 남아있는 상태라 앱에서 실제로 쓰이지 않음. `AGENTS.md`/`project-notes.md`에도 동일하게 반영.
...
auth.users
    ...
    └─→ profiles (1:1)
    │
    ├─→ posts (1:N) — 파티 구인 글 [레거시, 앱에서 미사용]
    │     └─→ applications (1:N) — 파티 신청 [레거시, 앱에서 미사용]
    │
...
### applications (파티 신청) — 레거시, 앱에서 미사용
| 컬럼 | 타입 | 설명 |
|---|---|---|
| id | UUID PK | |
