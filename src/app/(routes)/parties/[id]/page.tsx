...
export default function PartyDetailPage() {
  ...
  if (!post) {
    return <p className="text-sm text-slate-700">로딩 중...</p>;
  }
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p className="mt-2 text-slate-700">서버: {post.server}</p>
        <p className="text-slate-700">사냥터: {post.hunt_area}</p>
        <p className="text-slate-700">레벨: {post.level_min} - {post.level_max}</p>
        <p className="text-slate-700">인원: {post.slots_filled}/{post.slots_total}</p>
        <p className="text-slate-700">상태: {post.status}</p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        />
        <button type="button" className="mt-3 rounded-md bg-slate-900 px-4 py-2 text-white hover:bg-slate-700" onClick={apply}>
          신청
        </button>
      </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-3 space-y-3">
            {applications.length === 0 ? (
              <p className="text-sm text-slate-700">신청이 없습니다.</p>
            ) : (
              applications.map((application) => (
                <div key={application.id} className="rounded-md border border-slate-200 p-3">
                  <p className="text-sm">{application.message || "(메시지 없음)"}</p>
                  <p className="mt-1 text-xs text-slate-600">상태: {application.status}</p>
                  {application.status === "pending" && (
                    <div className="mt-2 flex gap-2">
                      ...
                      <button
                        type="button"
                        className="rounded-md bg-rose-700 px-3 py-1 text-sm text-white"
      ...
      {errorText && <p className="text-sm text-red-600">오류: {errorText}</p>}
    </section>
  );
}
