...
type OneHitCalculatorClientProps = {
  monsters: Monster[];
  server: "mapleland" | "planet";
};

export function OneHitCalculatorClient({ monsters, server }: OneHitCalculatorClientProps) {
  const typedMonsters = monsters;
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(profileStorageKey, JSON.stringify(profileSnapshot));
  }, [profileSnapshot, profileStorageKey]);
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert(
          ...
          { onConflict: "user_id,calculator,server,name" },
        );
  ...
  async function saveProfileForLoginUser() {
    try {
      ...
      window.localStorage.setItem(`${profileStorageKey}:${user.id}`, JSON.stringify(profileSnapshot));
            ...
            <Panel title="캐릭터 정보" tone="blue">
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid gap-2">
                    <label className="space-y-1">
                      ...
                      <input
                        className="h-[30px] w-full rounded-[6px] border border-[var(--retro-border)] bg-[var(--retro-cell)] px-2 text-xs text-[color:var(--retro-text)] transition duration-150 placeholder:text-slate-400/70 focus:border-[var(--brand-accent)] focus:outline-none focus:ring-1 focus:ring-[var(--brand-accent-soft)]"
                        value={nickname}
              ...
              <div className="rounded-[8px] border border-[var(--retro-border)] bg-[linear-gradient(180deg,rgba(56,189,248,0.12),rgba(56,189,248,0.03))] p-2">
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    className="inline-flex h-[34px] items-center rounded-[6px] border border-[var(--brand-accent)] bg-[var(--brand-accent-soft)] px-3 text-[11px] font-semibold text-[color:var(--brand-accent-text)] shadow-[0_4px_12px_rgba(0,0,0,0.2)] transition duration-150 hover:-translate-y-0.5 hover:bg-[var(--brand-accent-soft)] active:translate-y-0"
