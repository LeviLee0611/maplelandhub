...
function parseNpcMaps(html) {
  ...
  const seen = new Set();
  const regex =
    /<a class="search-page-add-content-box"[^>]*href="https:\/\/mapledb\.kr\/search\.php\?q=(\d+)&t=map"[^>]*>[\s\S]*?<h4[^>]*>\s*([^<]+?)\s*<\/h4>/g;
...
async function main() {
  ...
  console.log(`NPC rows: ${rows.length}, concurrency: ${CONCURRENCY}`);
